---
type: question
project: TermiX
date: 2026-07-27
text: When I connected my wallet to register for TermiX, Rabby Wallet showed the warning "The transaction is not associated with the website that initiated it." That made me hesitant, so I decided not to complete the registration. Could the admin explain why this warning appears? Is there a way to adjust the registration flow so this warning no longer appears in Rabby Wallet?
---
type: signal
project: Tori Finance
date: 2026-07-27
text: Tori Finance launched strUSD, a yield-bearing token backed by institutional delta-neutral FX carry strategies (borrow low-rate, lend high-rate, hedged back to USD), promoted at ~12% APY with no crypto market exposure. Day-one DeFi integrations went live on Pendle, Morpho, Royco, and Curve, and the $50M pre-deposit cap was lifted. Source: https://x.com/tori_finance/status/2081758481057784130
---
