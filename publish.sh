...
# 3) fix sitemap: @astrojs/sitemap creates sitemap-index.xml, Google needs /sitemap.xml
python3 scripts/fix_sitemap.py
...
# 5) trigger Vercel deploy (build reads SUPABASE_ANON_KEY env, regenerates data, builds)
